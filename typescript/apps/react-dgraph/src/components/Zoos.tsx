...
const AnimalsInZoo = (animals: Animal[] | undefined, open: boolean) => {
  if (!animals) return;
  return (
    <Collapse in={open} timeout="auto" unmountOnExit>
      ...
      >
        {animals.map((animal) => {
          return <ListItem key={animal?.name}>{animal?.name}</ListItem>;
        })}
      </List>
    </Collapse>
  );
  ...
  return (
    <Box>
      ...
      >
        ...
        {zoos.map((zoo) => {
          const animals = AnimalsInZoo(zoo?.animals, open);
          return (
            <Box>
              <ListItemButton onClick={handleClick}>
                <ListItem key={zoo?.name}>{zoo?.name}</ListItem>
